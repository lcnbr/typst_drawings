...
#let dtedges2 =  (
 ...
 (vertices:(<IN>,<IE>),bend:43deg,..edgestyle,label:[#h(0.5mm)4]),
 (vertices:(<IE>,<IS>),bend:43deg,..edgestyle,label:[#h(0.5mm)4]),
 (vertices:(<IW>,<IN>),bend:43deg,..edgestyle,label:[#h(0.5mm)1]),
 (vertices:(<IS>,<IW>),bend:43deg,..edgestyle,label:[#h(0.5mm)1]),
  ...
  render: (grid, nodes, edges, options) => {
    fletcher.cetz.canvas({
      fletcher.draw-diagram(grid, nodes, edges, debug: options.debug)
        puncture((2.9,1.24),radius:0.16,stroke:1.5pt+blue)    
    })
  }
),baseline: 0mm)