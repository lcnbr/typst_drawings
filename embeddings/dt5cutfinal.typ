 ...
 
 {
  ...
  render: (grid, nodes, edges, options) => {
    ...
    fletcher.cetz.canvas({
        ...
        let initial = (thickness:0.5mm,paint:red.lighten(0%),cap:"round")
        let initial1 = (thickness:0.5mm,paint:red.darken(20%),cap:"round")
        let initial2 = (thickness:0.5mm,paint:red.darken(40%),cap:"round")
        
        let r1p = (2.55*space,.65*spacey)
        puncture(r1p,stroke:initial,radius:0.14)

        angled_flag(r1p,-90deg,2.5*space,stroke:initial,label:[],flip:true)
        flag(r1p,(rel:(120deg,1.1)),(rel:(0deg,1)),(rel:(-60deg,1.)),(rel:(-15deg,2.)),initial1,size:0.16)
        flag(r1p,(rel:(50deg,.6)),(rel:(180deg,1)),(rel:(-120deg,.5)),(rel:(195deg,2.)),initial2,size:0.16)
