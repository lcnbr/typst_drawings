...
#import "../edge_style.typ":mom_arr ,initial
...
,{
  ...
  edge(<IWu>,<INl>,bend:31deg,..mom_arr,label:[$q-k$],label-angle:40deg,label-sep:0mm)
  edge(<ISl>,<IWd>,bend:31deg,..mom_arr,label:[$-k$],label-sep:-1mm)
  
  edge(<IEd>,<ISr>,bend:31deg,..mom_arr,label:[$p$],label-sep:-1mm)
  edge(<INr>,<IEu>,bend:31deg,..mom_arr,label:[$p+q$],label-angle:-40deg,label-sep:0mm)
  
  edge(<ISu>,<INd>,..mom_arr,shift:2mm,label:[$p+k$],label-angle:90deg,label-side:left,label-sep:0mm)
  ...
  render: (grid, nodes, edges, options) => {
    ...
    fletcher.cetz.canvas({
      ...
      puncture(r2p,stroke:initial,radius:0.16)
