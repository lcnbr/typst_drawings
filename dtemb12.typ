...
#set page(height: 11cm,width: 12cm)
#set text(size: 13pt)
  ...
  render: (grid, nodes, edges, options) => {
    ...
    fletcher.cetz.canvas({
      fletcher.draw-diagram(grid, nodes, edges, debug: options.debug)
        ...
        angled_flag(r1p,-45deg,3.5*space,stroke:tstroke,label:[$r_1+t_1-t_2+t_3$])
        angled_flag(r1p,45deg,3.5*space,stroke:tstroke,label:[$r_1+t_1-t_2$])
        angled_flag(r1p,145deg,3.*space,stroke:tstroke,label:[$r_1+t_1-t_2$],flip:true)
        angled_flag(r1p,-145deg,3.*space,stroke:stroke,label:[$r_1$],flip:true)
